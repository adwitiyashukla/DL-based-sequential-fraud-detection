...
│   └── export_space.py        builds the demo payload for the live Space
├── space/                     source of the Hugging Face Space
│   ├── app.py                 Gradio app, live inference on both models
│   ├── model.py               architecture copy, so the Space stands alone
│   ├── requirements.txt       CPU-only torch pin for the Space image
│   ├── README.md              Space card
│   └── assets/                gitignored, generated by src/export_space.py
└── results/
    ├── pr_curve.png
    ├── cost_curve.png
...
### Rebuilding the live demo

The Space serves real inference: the trained GRU checkpoint and the LightGBM booster are both
loaded and run on request, rather than scores being looked up. Its data payload is generated
from the trained models:

```bash
python -m src.export_space     # writes space/assets, about 7 MB
hf upload adwitiyashukla/sequential-fraud-detection space/ . --repo-type=space
```

The export curates 231 real test transactions across five scenarios, including the case worth
seeing, where the sequence model catches fraud the flat model clears. Each ships with its
10 step window pre-sliced, so the forward pass happens on click. The full test scores, labels
and amounts also go along, which is what lets the cost explorer recompute the confusion matrix
at any threshold instantly.

Assets are gitignored because they are derived artefacts, regenerable in one command.
